...
class ModelTfidfNaive(ModelPipeline):
    ...
    def predict_with_super_documents(self, x_test: np.ndarray) -> np.ndarray:
        '''Predictions with super documents
        Args:
            x_test (np.ndarray): Array-like or sparse matrix, shape = [n_samples, n_features]
        Raises:
            ValueError: If with_super_documents is not True
        Returns:
            (np.ndarray): Array, shape = [n_samples, n_classes]
        '''
        if not self.with_super_documents:
            raise ValueError("with_super_documents must be True in order to use the method predict_with_super_documents")
        else:
    ...
    @utils.trained_needed
    def predict_without_super_documents(self, x_test:np.ndarray) -> np.ndarray:
        ...
        if self.with_super_documents:
            raise ValueError("with_super_documents must be False in order to use the method predict_without_super_documents")
        else:
            chunk_size = 5000
    ...
    def predict(self, x_test, return_proba: bool = False, **kwargs) -> np.ndarray:
        ...
        if not return_proba:
            if self.with_super_documents:
                return self.predict_with_super_documents(x_test)
            else:
                return self.predict_without_super_documents(x_test)
        else:
