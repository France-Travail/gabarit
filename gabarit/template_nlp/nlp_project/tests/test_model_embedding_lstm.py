...
class ModelEmbeddingLstmTests(unittest.TestCase):
    '''Main class to test model_embedding_lstm'''
    ...
    def check_weights_equality(self, model_1, model_2):
        ...
        for layer_nb, x1, x2 in [(5, 1, 100), (9, 42, 132)]:
            self.assertAlmostEqual(model_1.model.weights[layer_nb].numpy()[x1, x2], model_2.model.weights[layer_nb].numpy()[x1, x2])

    def test01_model_embedding_lstm_init(self):
        '''Test of {{package_name}}.models_training.models_tensorflow.model_embedding_lstm.ModelEmbeddingLstm.__init__'''
        model_dir = os.path.join(os.getcwd(), 'model_test_123456789')
        remove_dir(model_dir)
        # Init., test all parameters
        model = ModelEmbeddingLstm(model_dir=model_dir)
        ...
        #
        model = ModelEmbeddingLstm(model_dir=model_dir, max_sequence_length=20)
        self.assertEqual(model.model_dir, model_dir)
        self.assertTrue(os.path.isdir(model_dir))
        ...
        #
        model = ModelEmbeddingLstm(model_dir=model_dir, max_words=100)
        self.assertEqual(model.model_dir, model_dir)
        self.assertTrue(os.path.isdir(model_dir))
        ...
        #
        model = ModelEmbeddingLstm(model_dir=model_dir, padding='post')
        self.assertEqual(model.model_dir, model_dir)
        self.assertTrue(os.path.isdir(model_dir))
        ...
        #
        model = ModelEmbeddingLstm(model_dir=model_dir, truncating='pre')
        self.assertEqual(model.model_dir, model_dir)
        self.assertTrue(os.path.isdir(model_dir))
        ...
        #
        model = ModelEmbeddingLstm(model_dir=model_dir, tokenizer_filters="!;/")
        self.assertEqual(model.model_dir, model_dir)
        self.assertTrue(os.path.isdir(model_dir))
        ...
        with self.assertRaises(ValueError):
            model = ModelEmbeddingLstm(model_dir=model_dir, padding='toto')
        with self.assertRaises(ValueError):
            model = ModelEmbeddingLstm(model_dir=model_dir, truncating='toto')

    def test02_model_embedding_lstm_predict_proba(self):
        '''Test of {{package_name}}.models_training.models_tensorflow.model_embedding_lstm.ModelEmbeddingLstm.predict_proba'''
    ...
    def test03_model_embedding_lstm_prepare_x_train(self):
        '''Test of {{package_name}}.models_training.models_tensorflow.model_embedding_lstm.ModelEmbeddingLstm._prepare_x_train'''
    ...
    def test04_model_embedding_lstm_prepare_x_test(self):
        '''Test of {{package_name}}.models_training.models_tensorflow.model_embedding_lstm.ModelEmbeddingLstm._prepare_x_test'''
    ...
    def test05_model_embedding_lstm_get_model(self):
        '''Test of {{package_name}}.models_training.models_tensorflow.model_embedding_lstm.ModelEmbeddingLstm._get_model'''
        ...
        model2 = ModelEmbeddingLstm(model_dir=model_dir, batch_size=8, epochs=2, multi_label=False,
                                  max_sequence_length=10, max_words=100, random_seed=42,
                                  padding='pre', truncating='post',
                                  embedding_name='fake_embedding.pkl')
        ...
        model1._prepare_x_train(x_train)
        model1.list_classes = ['a', 'b']
        model2 = ModelEmbeddingLstm(model_dir=model_dir, batch_size=8, epochs=2, multi_label=True,
                                  max_sequence_length=10, max_words=100, random_seed=42,
                                  padding='pre', truncating='post',
                                  embedding_name='fake_embedding.pkl')
        model2._prepare_x_train(x_train)
    ...
    def test06_model_embedding_lstm_save(self):
        '''Test of the method save of {{package_name}}.models_training.models_tensorflow.model_embedding_lstm.ModelEmbeddingLstm'''
        ...
        self.assertTrue(os.path.exists(os.path.join(model.model_dir, 'model_embedding_lstm.pkl')))
        ...
        self.assertTrue(os.path.exists(os.path.join(model.model_dir, 'configurations.json')))
        # self.assertTrue(os.path.exists(os.path.join(model.model_dir, 'best.hdf5'))) -> no model trained
        self.assertFalse(os.path.exists(os.path.join(model.model_dir, 'model_embedding_lstm.pkl')))
        self.assertFalse(os.path.exists(os.path.join(model.model_dir, 'embedding_tokenizer.pkl')))
        with open(os.path.join(model.model_dir, 'configurations.json'), 'r', encoding='{{default_encoding}}') as f:
            configs = json.load(f)
        self.assertEqual(configs['test'], 8)
    ...
    def test07_model_keras_init_new_instance_from_configs(self):
        '''Test of the method _init_new_instance_from_configs of {{package_name}}.models_training.models_tensorflow.model_embedding_lstm.ModelEmbeddingLstm'''
        model_dir = os.path.join(os.getcwd(), 'model_test_123456789')
        remove_dir(model_dir)
        ...
        new_model = ModelEmbeddingLstm._init_new_instance_from_configs(configs=configs)
        self.assertTrue(isinstance(new_model, ModelEmbeddingLstm))
    ...
    def test08_model_keras_load_standalone_files(self):
        '''Test of the method _load_standalone_files of {{package_name}}.models_training.models_tensorflow.model_embedding_lstm.ModelEmbeddingLstm'''
        model_dir = os.path.join(os.getcwd(), 'model_test_123456789')
        remove_dir(model_dir)
        ...
        model = ModelEmbeddingLstm(model_dir=model_dir, embedding_name='fake_embedding.pkl')
