...
from {{package_name}}.models_training.models_sklearn.model_tfidf_sgdc import ModelTfidfSgdc
...
class ModelTfidfSgdcTests(unittest.TestCase):
    ...
    def test01_model_tfidf_sgdc_init(self):
        '''Test of {{package_name}}.models_training.models_sklearn.model_tfidf_sgdc.ModelTfidfSgdc.__init__'''
    ...
    def test02_model_tfidf_sgdc_predict(self):
        '''Test of the method predict of {{package_name}}.models_training.models_sklearn.model_tfidf_sgdc.ModelTfidfSgdc'''
    ...
    def test03_model_tfidf_sgdc_predict_proba(self):
        '''Test of {{package_name}}.models_training.models_sklearn.model_tfidf_sgdc.ModelTfidfSgdc.predict_proba'''
        ...
        remove_dir(model_dir)
        model = ModelTfidfSgdc(model_dir=model_dir, multi_label=False, multiclass_strategy=None, sgdc_params={'loss': 'log_loss'})
        model.fit(x_train, y_train_mono)
        preds = model.predict_proba(x_train)
        self.assertEqual(preds.shape, (len(x_train), n_classes))
        preds = model.predict_proba('test')
        ...
        self.assertEqual([elem for elem in preds], [elem for elem in model.predict_proba(['test'])[0]])
        remove_dir(model_dir)
        model = ModelTfidfSgdc(model_dir=model_dir, multi_label=True, multiclass_strategy=None, sgdc_params={'loss': 'log_loss'})
        model.fit(x_train, y_train_multi[cols])
        preds = model.predict_proba(x_train)
        self.assertEqual(preds.shape, (len(x_train), len(cols)))
        preds = model.predict_proba('test')
    ...
    def test04_model_tfidf_sgdc_get_predict_position(self):
        '''Test of the method {{package_name}}.models_training.models_sklearn.model_tfidf_sgdc.ModelTfidfSgdc.get_predict_position'''
    ...
    def test_06_model_tfidf_sgdc_init_new_instance_from_configs(self):
        '''Test of the method {{package_name}}.models_training.models_sklearn.model_tfidf_sgdc.ModelTfidfSgdc._init_new_instance_from_configs'''
        model_dir = os.path.join(os.getcwd(), 'model_test_123456789')
        remove_dir(model_dir)
    ...
    def test_07_model_tfidf_sgdc__load_standalone_files(self):
        '''Test of the method {{package_name}}.models_training.models_sklearn.model_tfidf_sgdc.ModelTfidfSgdc._load_standalone_files'''
        model_dir = os.path.join(os.getcwd(), 'model_test_123456789')
        remove_dir(model_dir)
        ...
        new_model = ModelTfidfSgdc(model_dir=new_model_dir)
