...
class ModelAggregationRegressorTests(unittest.TestCase):
    ...
    def test01_model_aggregation_regressor_init(self):
        ...
        test_init_partial(model, model_name, model_dir, list_model_names, [False, False])
    ...
    def test08_model_aggregation_regressor_median_predict(self):
        ...
        self.assertAlmostEqual(model.median_predict(preds), np.median(preds))
        # 3 models float
    ...
    def test09_model_aggregation_regressor_mean_predict(self):
        ...
        # 3 models int
        preds = np.array([3, 4, 4])
        self.assertAlmostEqual(model.mean_predict(preds), np.mean(preds))
        # 3 models float
        preds = np.array([3, 3.1, 4])
        self.assertAlmostEqual(model.mean_predict(preds), np.mean(preds))
        # 1 model int
    ...
    def test12_model_aggregation_regressor_reload_from_standalone(self):
