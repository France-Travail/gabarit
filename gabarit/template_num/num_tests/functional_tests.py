...
class Case1_e2e_pipeline(unittest.TestCase):
    ...
    def test01_CreateSamples(self):
        ...
        # Same random_seed
        double_files_run_1 = f"{activate_venv}python {full_path_lib}/test_template_num-scripts/utils/0_create_samples.py --overwrite -f mono_class_mono_label_1.csv multi_class_mono_label_1.csv -n 2000 -s 42"
        self.assertEqual(subprocess.run(double_files_run_1, shell=True).returncode, 0)
        double_files_run_2 = f"{activate_venv}python {full_path_lib}/test_template_num-scripts/utils/0_create_samples.py --overwrite -f mono_class_mono_label_2.csv multi_class_mono_label_2.csv -n 2000 -s 42"
        self.assertEqual(subprocess.run(double_files_run_2, shell=True).returncode, 0)
        df1_mono = pd.read_csv(f"{full_path_lib}/test_template_num-data/mono_class_mono_label_1_2000_samples.csv", sep=';', encoding='utf-8')
        df1_multi = pd.read_csv(f"{full_path_lib}/test_template_num-data/multi_class_mono_label_1_2000_samples.csv", sep=';', encoding='utf-8')
        df2_mono = pd.read_csv(f"{full_path_lib}/test_template_num-data/mono_class_mono_label_2_2000_samples.csv", sep=';', encoding='utf-8')
        df2_multi = pd.read_csv(f"{full_path_lib}/test_template_num-data/multi_class_mono_label_2_2000_samples.csv", sep=';', encoding='utf-8')
        ...
        double_files_run_1 = f"{activate_venv}python {full_path_lib}/test_template_num-scripts/utils/0_create_samples.py --overwrite -f mono_class_mono_label_1.csv multi_class_mono_label_1.csv -n 2000 -s 42"
        self.assertEqual(subprocess.run(double_files_run_1, shell=True).returncode, 0)
        double_files_run_2 = f"{activate_venv}python {full_path_lib}/test_template_num-scripts/utils/0_create_samples.py --overwrite -f mono_class_mono_label_2.csv multi_class_mono_label_2.csv -n 2000 -s 41"
        self.assertEqual(subprocess.run(double_files_run_2, shell=True).returncode, 0)
        df1_mono = pd.read_csv(f"{full_path_lib}/test_template_num-data/mono_class_mono_label_1_2000_samples.csv", sep=';', encoding='utf-8')
        df1_multi = pd.read_csv(f"{full_path_lib}/test_template_num-data/multi_class_mono_label_1_2000_samples.csv", sep=';', encoding='utf-8')
        df2_mono = pd.read_csv(f"{full_path_lib}/test_template_num-data/mono_class_mono_label_2_2000_samples.csv", sep=';', encoding='utf-8')
...
class Case2_MonoClassMonoLabel(unittest.TestCase):
    ...
    def test01_PrepareDatasets(self):
        ...
        preprocessing = f"{activate_venv}python {full_path_lib}/test_template_num-scripts/1_preprocess_data.py --overwrite -f mono_class_mono_label_train.csv -p preprocess_P1 --target_cols y_col"
...
class Case3_MonoClassMultiLabel(unittest.TestCase):
    ...
    def test01_PrepareDatasets(self):
        ...
        preprocessing = f"{activate_venv}python {full_path_lib}/test_template_num-scripts/1_preprocess_data.py --overwrite -f mono_class_multi_label_train.csv -p preprocess_P1 --target_cols y_col_1 y_col_2"
        # We don't apply the preprocessing on the validation dataset. We will use the train dataset as validation to simplify
        self.assertEqual(subprocess.run(split_train_valid_test, shell=True).returncode, 0)
        self.assertEqual(subprocess.run(preprocessing, shell=True).returncode, 0)
...
class Case4_MultiClassMonoLabel(unittest.TestCase):
    ...
    def test01_PrepareDatasets(self):
        ...
        preprocessing = f"{activate_venv}python {full_path_lib}/test_template_num-scripts/1_preprocess_data.py --overwrite -f multi_class_mono_label_train.csv -p preprocess_P1 --target_cols y_col"
        # We don't apply the preprocessing on the validation dataset. We will use the train dataset as validation to simplify
        self.assertEqual(subprocess.run(split_train_valid_test, shell=True).returncode, 0)
        self.assertEqual(subprocess.run(preprocessing, shell=True).returncode, 0)
...
class Case5_MonoOutputRegression(unittest.TestCase):
    ...
    def test14_Model_AggregationRegressor(self):
        ...
        try:
            ...
            model_name = 'aggregation_multi_class_mono_label'
            model_name_sgd1 = 'aggregation_multi_class_mono_label_sgd1'
            model_name_sgd2 = 'aggregation_multi_class_mono_label_sgd2'
            model_name_gbt = 'aggregation_multi_class_mono_label_gbt'
            model_dir_sgd1 = os.path.join(utils.get_models_path(), model_name_sgd1, datetime.now().strftime(f"{model_name_sgd1}_%Y_%m_%d-%H_%M_%S"))
            model_dir_sgd2 = os.path.join(utils.get_models_path(), model_name_sgd2, datetime.now().strftime(f"{model_name_sgd2}_%Y_%m_%d-%H_%M_%S"))
            model_dir_gbt = os.path.join(utils.get_models_path(), model_name_gbt, datetime.now().strftime(f"{model_name_gbt}_%Y_%m_%d-%H_%M_%S"))
            model_dir = os.path.join(utils.get_models_path(), model_name, datetime.now().strftime(f"{model_name}_%Y_%m_%d-%H_%M_%S"))
